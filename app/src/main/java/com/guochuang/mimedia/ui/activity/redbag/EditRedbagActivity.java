...
//                money=0.01d;
    ...
    public void startPay(String safetyCode) {
        ...
        if (TextUtils.equals(redPacketType, Constant.RED_PACKET_TYPE_RANDOM)) {
            ...
            //picture 拼接的是视频地址
            // TODO: 2019/3/9
            String problemJson = joinJson();
            if (TextUtils.isEmpty(problemJson)) {
                showShortToast("请设置问题");
                return;
            }

            mvpPresenter.addVideoReabag(latitude, longitude, redbagLatitude, redbagLongitude, content, picture, areaType, kilometer, money, quantity, urlName, url, wechat, microblog, isPublicPassword, isSaveTemplate, payType, Constant.CHANNEL_CODE_ANDROID, safetyCode, problemJson);
        }
    }
    ...
    private String joinJson() {
        ...
        JSONArray problemJsonArray = new JSONArray();
        ...
        for (int i = 0; i < mProblemList.size(); i++) {
            ...
            try {
                ...
                JSONArray problemItemJsonArray = new JSONArray();
                ArrayList<ProblemBean.ItemBean> item = mProblemList.get(i).getItem();
                //item 的条目
                for (int j = 0; j < item.size(); j++) {
                    JSONObject itemJsonObject = new JSONObject();
                    ProblemBean.ItemBean itemBean = item.get(j);
                    itemJsonObject.put("optionName", itemBean.getItemname())
                    ...
                    problemItemJsonArray.put(itemJsonObject);

                }


                problemJson.put("surveyId", "")
                        .put("title", problemBean.getProblem())
                        .put("type", problemBean.getType())
                        .put("sequence", i)
                        .put("optionsList", problemItemJsonArray);

                problemJsonArray.put(problemJson);
        ...
        LogUtil.json(problemJsonArray.toString());
        return problemJsonArray.toString();
    }
