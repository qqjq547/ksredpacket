...
//            }
    ...
    private void highlightPoints(Canvas canvas) {
        ...
        for (Line line:dataProvider.getLineChartData().getLines()){
            drawPoints(canvas, line, 0, MODE_HIGHLIGHT);
        }
    }
    ...
    private void drawLabel(Canvas canvas, Line line, PointValue pointValue, float rawX, float rawY, float offset) {
        ...
        String[] lines= String.valueOf(pointValue.getLabelAsChars()).split("\n");
        float labelWidth;
        if (lines.length>1){
            float lineWidth1=labelPaint.measureText(lines[0]);
            float lineWidth2=labelPaint.measureText(lines[1]);
            labelWidth=lineWidth1>lineWidth2?lineWidth1:lineWidth2;
        }else {
            float lineWidth1=labelPaint.measureText(lines[0]);
            labelWidth=lineWidth1;
        }
//        final float labelWidth = labelPaint.measureText(labelBuffer, labelBuffer.length - numChars, numChars);
        final int labelHeight = Math.abs(fontMetrics.ascent)*2;
        ...
        drawLabelTextAndBackground(canvas, pointValue.getLabelAsChars(), pointValue.getLabelAsChars().length - numChars, numChars,
                line.getPointColor());
    }
...
//        return Math.pow(diffX, 2) + Math.pow(diffY, 2) <= 2 * Math.pow(radius, 2);
          return touchX>(x-radius)&&touchX<(x+radius);
    }
